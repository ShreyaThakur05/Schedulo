...
function GCalCard() {
  ...
  const handleConnect = async () => {
    try { const res = await api.get('/gcal/auth-url'); window.location.href = res.data.data.url; }
    catch { toast('Google Calendar not configured on server.', 'error'); }
  };

  const handleDisconnect = async () => { await disconnect.mutateAsync(); toast('Google Calendar disconnected.'); refetch(); };

  if (isLoading) return <div className="flex items-center gap-2 text-sm text-indigo-400"><Spinner className="h-4 w-4" /> Loading…</div>;
  ...
  return (
    <div className="flex items-center justify-between flex-wrap gap-4">
      ...
      {data?.connected
        ? <button onClick={handleDisconnect} disabled={disconnect.isPending} className="px-4 py-2 text-sm font-medium rounded-xl border border-red-200 text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50">Disconnect</button>
        : <button onClick={handleConnect} className="px-4 py-2 text-sm font-semibold rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white transition-colors shadow-sm shadow-indigo-200 dark:shadow-none">Connect Google Calendar</button>}
    </div>
  );
}
      ...
      <div className="flex flex-col gap-6 max-w-2xl">
        ...
        <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-2xl p-5">
          ...
          <ol className="text-xs text-amber-700 dark:text-amber-500 space-y-1 list-decimal list-inside">
            <li>Go to <strong>Google Cloud Console</strong> → Create OAuth 2.0 credentials</li>
            <li>Set redirect URI to <code className="bg-amber-100 dark:bg-amber-900/30 px-1 rounded">http://localhost:4000/api/v1/gcal/callback</code></li>
            <li>Add <code className="bg-amber-100 dark:bg-amber-900/30 px-1 rounded">GOOGLE_CLIENT_ID</code> and <code className="bg-amber-100 dark:bg-amber-900/30 px-1 rounded">GOOGLE_CLIENT_SECRET</code> to <code className="bg-amber-100 dark:bg-amber-900/30 px-1 rounded">server-js/.env</code></li>
            <li>Restart the server and click Connect above</li>
          </ol>
        </div>
      </div>
    </>
